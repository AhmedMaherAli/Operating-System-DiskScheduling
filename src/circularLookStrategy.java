...
public class circularLookStrategy implements schedulingStrategy{
	private int []requests;
	private int myInitialPos;
	public binarySearch binarysearch;
	circularLookStrategy()
	{
	...
	public void execute(int arr[],int initialPos,String direction) {
		...
		Vector<Integer>sequence=new Vector<Integer>();
		{
			...
			if(direction.equals("left"))
			{
				...
				totalDistance+=(requests[requests.length-1]);
				myInitialPos=requests[requests.length-1];
				for(int i=requests.length-2;i>=start;i--)
				{
			...
			else
			{
				...
				if(start!=0) {
				totalDistance+=(Math.abs(myInitialPos-requests[0]));
				sequence.add(requests[0]);
				myInitialPos=requests[0];
				}
				...
				System.out.println("The total distance for circular look algorithm is: "+totalDistance);
				System.out.println("The requests sequnce is: ");
				for(int i=0;i<sequence.size();i++)
				{
					System.out.print(sequence.elementAt(i)+" ");
